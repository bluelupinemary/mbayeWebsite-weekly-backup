function blackhole(element, maxOrbit, positionY) {
	...
	var star = function(){
		...
		this.speed = (Math.floor(Math.random() * 4.5) + 1.5)*Math.PI/180; // The rate at which this star will orbit
	...
	$('.decline-friend-icon').on('click',function(){
		collapse = false;
		expanse = true;
		// $(this).closest('.friends').css('transform', 'translate(-50%, -50%) scale(0)');
		$(this).closest('.friends').addClass('rotate');
	});
