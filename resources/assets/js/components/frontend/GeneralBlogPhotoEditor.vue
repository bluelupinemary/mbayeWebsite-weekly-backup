...
export default {
    ...
    mounted() {
        ...
        $('.tui-image-editor-cancel-btn').click(function() {
            $('#generalBlogPhotoEditorModal').modal('hide');
        });
        ...
        $('.tui-image-editor-save-btn').click(function() {
            ...
            $('.general-blog-form input[name="edited_featured_image"]').val(image_src);
            $('.general-blog #featured-image-previewimg').attr('src', image_src);
            $('#generalBlogPhotoEditorModal').modal('hide').fadeOut();
        });
        ...
        // load image on change of featured image input file
        document.querySelector('#general_blog_featured_image').addEventListener('change',function(event) {
            const file = event.target.files[0];
            if (file) {
                var reader = new FileReader();
                reader.onload = function (e) {
