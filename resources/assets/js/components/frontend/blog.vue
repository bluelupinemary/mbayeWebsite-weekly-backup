...
<div>
            ...
            <div id="camera" class="view">
                <div id="dolly" class="view">
                    <div id="stack" class="view" v-touch:swipe.left="lefthandler" v-touch:swipe.right="righthandler">
                        <div v-for="(blog,index) in blogs" :key="index" :class="'cell fader view original div_img div_img_'+index" :block_no="index" v-for-callback="{key: index, array: blogs, callback: callback}" :style="blockStyle(index)" style="opacity: 0;">
                            <a class="mover viewflat blog_img" href="#" @click.prevent="playAudio('div_img_'+index, blog)">
                                ...
                                <img :class="'cell_img_'+index+' '+(blog.shared ? 'green-border' : '')" :src="'/storage/img/'+blog.thumb" @load="layoutImageInCell('cell_img_'+index, index)" :data-index="index">
                            </a>
                                
                            <div class="overlay">
                                <div class="blog-title">
                                    <div v-if="blog.shared == false" :class="'div_title_'+index+' div_title text_left'" style="display:none;z-index:10000000;border:0px solid white;width:90%;">
                                ...
                                <button v-if="blog.shared" class="button btn_view_blog" @click.prevent="viewSharedBlog(blog.shared_id)"><p class="p_button">View Blog</p></button>
                                <button v-else class="button btn_view_blog" @click.prevent="viewBlog(blog.id)"><p class="p_button">View Blog</p></button>
                            </div>
                            <li v-if="blog.shared" :class="'tag tag_'+index+' front'"><i class="fas fa-tag"></i> Shared</li>
                            <div :class="'div_count_bg'+index+' div_count_bg div_count_regular '+blog.most_reaction" @click.prevent="playAudio('div_img_'+index, blog)">
                            ...
                            <div :class="'div_count_bg'+index+' div_count_bg div_count_small '+blog.most_reaction" @click.prevent="playAudio('div_img_'+index, blog)">
    ...
    <div class="title">
        <p v-if="user_id != 0 && type == ''">Your<br><span v-if="formattedTag != ''"> {{formattedTag}} </span>Blogs</p>
        <p v-else-if="user_id != 0 && type == 'friend'">{{friend.first_name}}'s<br><span v-if="formattedTag != ''"> {{formattedTag}} </span>Blogs</p>
        <p v-else-if="user_id == 0 && type == 'friend'">Your Friends'<br><span v-if="formattedTag != ''"> {{formattedTag}} </span>Blogs</p>
        <p v-else>Other Earthlings'<br><span v-if="formattedTag != ''"> {{formattedTag}} </span>Blogs</p>
    </div>
    <div v-if="user" :class="'astro-div navigator-div '+getUserGender(user.gender)">
        ...
        <div :class="'tag-title '+(user.gender != null ? user.gender: 'male')+' '+tag">
            ...
            <img v-else-if="tag == 'careers'" :src="'/front/images/planets/Mercury.png'"/>
        </div>
...
export default {
    ...
    computed: {
        formattedTag() {
            if(this.tag != '') {
                if(this.tag == 'films' || this.tag == 'careers') {
                    if(this.tag == 'films') {
                        return 'Film';
                    } else if(this.tag == 'careers') {
                        return 'Career';
                    }
                } else {
                    var tag_title = this.tag.split('_');

                    if(tag_title.length > 1) {
                        return this.ucfirst(tag_title[0])+' and '+this.ucfirst(tag_title[2]);
                    } else {
                        return this.ucfirst(tag_title[0]);
                    }
                }
    ...
    mounted () {
        ...
        .listen('NewBlogEvent',(response) => {
            ...
            if(that.tag != '' && response.blog_tags.length > 0) {
                if(response.blog_tags.includes(that.tag) ) {
                    if(that.user_id == 0) {
        ...
        .listen('NewBlogShareEvent',(response) => {
            // console.log(response);
    ...
    methods:{
            ...
            .then((response) => {
                ...
                $.each(response.data.data, function(index, value) {
                    if(value.blog) {
                        ...
                        that.blogs[index].shared_id = value.id;
                    } else {
                    ...
                    // that.blogs[index].hotcount = 556;
                    // that.blogs[index].coolcount = 556;
                    // that.blogs[index].naffcount = 556;
                    // that.blogs[index].commentcount = 556;
                    // i++;
                    that.changes(index);
                });
        ...
        countemotions(index) {
            ...
            .then((response) => {
                ...
                this.blogs[index].most_reaction= response.data.most_reaction;
            })
        ...
        { 
            ...
            if (scroll=='left') {
                ...
                if(that.page==that.last_page) {
                    ...
                    // console.log(that.Total_count, that.blogs.length)
                    if(that.Total_count == that.blogs.length && (that.newcell+3) >= (that.cellCount)) {
        ...
        viewSharedBlog(shared_id) {
            window.location.href = '/shared_blog/'+shared_id;
        },
        getAudio()
        {
