...
@section ('title', trans('labels.backend.generalblogs.management'))

@section('page-header')
    <h1>{{ trans('labels.backend.generalblogs.management') }}</h1>
@endsection

@section('content')
    <div class="box box-info">
        <div class="box-header with-border">
            <h3 class="box-title">{{ trans('labels.backend.generalblogs.management') }}</h3>
    ...
    <script>
        $(function() {
            ...
            var dataTable = $('#blogs-table').dataTable({
                ...
                serverSide: true,
                ajax: {
                    url: '{{ route("admin.generalblogs.get") }}',
                    type: 'post'
                },
                columns: [
                    {data: 'name', name: '{{config('module.general_blogs.table')}}.name'},
                    {data: 'publish_datetime', name: '{{config('module.general_blogs.table')}}.publish_datetime'},
                    {data: 'status', name: '{{config('module.general_blogs.table')}}.status'},
                    {data: 'created_by', name: '{{config('module.general_blogs.table')}}.created_by'},
                    {data: 'Email', name: 'email'},
                    {data: 'shares', name: 'shares'},
                    {data: 'created_at', name: '{{config('module.general_blogs.table')}}.created_at'},
